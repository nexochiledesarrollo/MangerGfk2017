...
public class FileUploadM5 implements AccessFileUploadM5 {
		
	private static final Logger logger = Logger.getLogger(FileUploadM5.class);
    ...
    public ArrayList<ObjUploadFile> getlistFileUpload(int id) {
		...
		String query=" SELECT f.id_file "
					  ...
					  +" WHERE f.tipo_documental = 2 "
					  +" AND f.id_operacion = " +id+" "
					  +" order by f.id_file DESC ";
		
		 logger.debug("QUERY LIST CUESTIONARIO: " + query); 
    ...
    public int getLastVersionPUpload(int op, int tipo) {
		...
		String query="SELECT MAX(vm_file)AS id "
					 +" FROM man_file_upload "
					 +" WHERE id_operacion = " + op +" "
					 +" AND tipo_documental = "+tipo+" ";
		 try {
			  conn = dataSource.getConnection();
			  PreparedStatement ps = conn.prepareStatement(query);
			  ResultSet rs = ps.executeQuery();
	...
	public int updateStateVfile(int op, int tipo, int state){
		Connection conn = null;
		String query = " UPDATE man_file_upload "
					   +"	   SET estadov_file ="+ state + " "
					   +" WHERE id_operacion = "+op+" "
					   +" AND tipo_documental = "+ tipo +" ";
